...
class OperacaoRepository(
    val operacaoDao: OperacaoDao,
) {
    fun getAllOperacoes(): Flow<List<Operacao>> = operacaoDao.getAllOperacoes()

    fun getOperacoesByCliente(clienteId: Long): Flow<List<Operacao>> = operacaoDao.getOperacoesByCliente(clienteId)

    fun getOperacoesByDateRange(
        startDate: Date,
        endDate: Date,
    ): Flow<List<Operacao>> = operacaoDao.getOperacoesByDateRange(startDate, endDate)
